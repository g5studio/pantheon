...
`evolve` 需在 `adapt` 完成後於**目標專案**執行；`project-schema` 與 `.evolve-tmp/` 等產物寫入目標專案，不在 Pantheon repo 內。詳見 `.cursor/commands/utilities/adapt.md` 與 `.cursor/commands/utilities/evolve.md`。
