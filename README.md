...
2. 將 Pantheon 的 `commands`、`scripts`、`skills`、`rules` 複製安裝到 `.cursor/*/{deities}/` 與 `.agents/*/{deities}/` 下
3. 將 `.cursor/rules/{deities}/` 與 `.agents/rules/{deities}/` 安裝到目標專案，保留 Cursor/Agent 規則能力
...
| `.pantheon/.cursor/rules/*.mdc` | `.cursor/rules/{deities}/*.mdc`、`.agents/rules/{deities}/*.mdc` |
