...
**注意**：只要是 `start-task` 觸發的流程都視為 AI 處理，會保存 start-task 資訊到 Git notes，供後續流程判斷（例如：建立 MR 時自動添加 `AI` label）。

**用戶確認後**：才能開始實際開發（但保存 Git notes 不以確認為前提）。

---

4. **🚨 保存 start-task 資訊到 Git notes（強制步驟）**：
   - **CRITICAL**: 只要是 `start-task` 觸發，**就必須保存** start-task 資訊到 Git notes（即使計劃尚未確認）
   - **補充**：若用戶確認計劃，應更新/覆蓋 Git notes，確保內容為最終版本
   - 使用腳本保存：
     ```bash
