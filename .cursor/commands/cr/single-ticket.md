...
**`--development-report` 必填區塊（CRITICAL）：**
- **所有類型都必填**：`## 📋 關聯單資訊`、`## 📝 變更摘要`、`## ⚠️ 風險評估`
- **Bug 類型必填**：`影響範圍`、`根本原因`、`造成問題的單號`、`改動前後邏輯差異`
- **Request 類型必填**：`預期效果`、`需求覆蓋率`、`潛在影響風險報告`
- **Agent Version 必填**：需傳入 `--agent-version`，由 `create-mr.mjs` 在 description 最下方追加 `### 🤖 Agent Version`
- **阻擋機制**：若 `development-report` 缺必要段落，`create-mr` / `update-mr` 會直接停止並回傳缺漏段落
...
**注意：** 
- `cr single-ticket` 指令預設會送審；如需不送審，請使用 `--no-review`。
