...
**重要**：檢查 Cursor rules 的處理方式遵循 [commit-rule-violation-handling.mdc](mdc:.cursor/rules/commit-rule-violation-handling.mdc) 規則。**必須同時遵守** [ai-decision-making-priorities.mdc](mdc:.cursor/rules/ai-decision-making-priorities.mdc) 規則：當檢測到需要修改代碼的問題時，**必須立即停止並詢問用戶**，不能自動修復。**先詢問再修改 > 完成任務**。
...
1. **關聯單號偵測**：如果提到了單號（例如：`IN-1234`、`FE-5678` 等），系統會自動偵測並詢問
   - 注意：`cr-single-ticket` 指令不支援 `--related-tickets` 參數，如果偵測到單號，系統會提示該指令不支援此功能，建議使用 `cr` 指令
