...
/**
 * 將 start-task-info.json 的 planConfirmed 設為 true（檔案化）
 *
 * 使用方式：
 *   node .cursor/scripts/operator/confirm-development-plan.mjs --ticket="FE-1234"
 *   node .cursor/scripts/operator/confirm-development-plan.mjs --start-task-dir=".cursor/tmp/FE-1234"
 *   node .cursor/scripts/operator/confirm-development-plan.mjs --start-task-info-file=".cursor/tmp/FE-1234/start-task-info.json"
 */
...
function main() {
  ...
  info.planConfirmed = true;
  info.planConfirmedAt = new Date().toISOString();
  writeFileSync(infoFile, JSON.stringify(info, null, 2), "utf-8");
  console.log("✅ 已確認開發計劃（planConfirmed=true）");
