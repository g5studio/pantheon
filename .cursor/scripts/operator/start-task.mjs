...
async function main() {
  ...
  try {
    ...
    const startTaskInfo = {
      ...
      // 用於區分「start-task 觸發」與「計劃是否已確認」
      planConfirmed: false,
    };

    const confirm = await question("❓ 請確認計劃是否正確？(y/N): ");
    startTaskInfo.planConfirmed = confirm.toLowerCase() === "y";

    if (startTaskInfo.planConfirmed) {
      console.log("\n✅ 計劃已確認，可以開始開發！\n");
    } else {
      console.log("\n💡 如需調整計劃，請告知具體需求\n");
    }

    // 🚨 不論是否確認計劃，start-task 觸發就必須保存 Git notes
    // 用於後續流程判斷（例如：建立 MR 時自動添加 AI label）
    try {
      ...
      if (result.status === 0) {
        console.log("💾 已保存 start-task 資訊到 Git notes\n");
      }
    } catch (error) {
      console.log(`⚠️  無法保存 start-task 資訊: ${error.message}\n`);
    }
  } catch (error) {
