...
function getJiraConfig() {
  ...
  if (!email || !apiToken) {
    console.error('\n❌ Jira 配置缺失！');
    console.error('   請在 .env.local 文件中設置以下環境變數：');
...
function analyzeTicketAndPlan(ticketData) {
  ...
  if (issueType.toLowerCase().includes('feature') || issueType.toLowerCase().includes('story')) {
  ...
  } else if (issueType.toLowerCase().includes('bug') || issueType.toLowerCase().includes('fix')) {
    analysis.suggestedSteps = [
      '1. 重現問題並定位根本原因',
      '2. 分析相關代碼邏輯',
      '3. 修復問題',
    ...
    analysis.suggestedSteps = ['1. 分析需求', '2. 設計實現方案', '3. 實現功能', '4. 測試驗證'];
  }
...
function branchExists(branchName) {
  try {
    exec(`git show-ref --verify --quiet refs/heads/${branchName}`, { silent: true });
    return true;
  } catch (error) {
    return false;
  }
...
async function main() {
  ...
  let ticket = '';
  while (!ticket) {
    ticket = await question('📋 請提供 Jira 單號（格式: FE-1234, IN-5678，必填）: ');
  ...
  try {
    ...
    if (branchExists(featureBranch)) {
      const switchBranch = await question(`分支 ${featureBranch} 已存在，是否切換? (y/N): `);
  ...
  try {
    ...
    if (confirm.toLowerCase() === 'y') {
      ...
      try {
        ...
        const result = spawnSync('git', ['notes', '--ref=start-task', 'add', '-f', '-F', '-'], {
          cwd: projectRoot,
          input: noteContent,
          encoding: 'utf-8',
