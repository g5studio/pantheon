...
// notifyCursorRulesFailed 函數已從 notify-cursor-rules-failed.mjs 導入
// 當 Cursor AI 在執行 commit 前檢測到 cursor rules 違規時，可以直接調用此函數
...
try {
  ...
  try {
    ...
    if (parentNote) {
      // 複製到當前 commit
      const result = spawnSync('git', ['notes', '--ref=start-task', 'add', '-f', '-F', '-', currentCommit], {
        cwd: projectRoot,
        input: parentNote,
        encoding: 'utf-8',
...
if (autoPush) {
  ...
  try {
    ...
    // 獲取 remote URL
    try {
      const remoteUrl = exec('git config --get remote.origin.url', { silent: true }).trim();
